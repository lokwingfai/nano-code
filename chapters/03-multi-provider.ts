...
import { createOllamaCloud } from '../src/providers/ollamacloud';
...
async function main() {
    ...
    const openaiModel = process.env.DEEPSEEK_MODEL_ID ?? 'deepseek-chat';
    ...
    const ollamaCloudModel = process.env.OLLAMA_MODEL_ID ?? 'gemma4:31b-cloud';
    ...
    try {
        const res2 = await generateText({ model: ollamaCloud(ollamaCloudModel), messages });
